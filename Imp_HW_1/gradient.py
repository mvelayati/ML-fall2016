...
def gradient_descent(x, y, w, p, l_rate, e):
    ...
    while (np.linalg.norm(gradient) > e) and (norm_gradient[-1] < 100000):
...
def part2():
    ...
    plt.ylabel(r'$\rm{log}_{10}(SEE)$', fontsize=10)
    plt.legend(loc='upper right', fontsize=10)
...
def part3():
    ...
    def ten_fold_cross_validation(dataset_x, dataset_y):
        ...
        for lambda_cof in sp:
            ...
            print("cross-validated SSE {} training SSE {}".format(test_loss, cost[-1]))
        return sse_lambda_array, sp_loss

    sse_lambda_array, sp_loss = ten_fold_cross_validation(x, y)
    print(sse_lambda_array)
    sse_min = min(sse_lambda_array)
    print("optimum lambda is {} corresponding SSE is {} ".format(sp[sse_lambda_array.index(sse_min)], sse_min))
    fig = plt.figure(figsize=(5, 5))
    plt.plot(sp, sse_lambda_array, '-k')
    plt.xscale('log')
    plt.yscale('log')
    plt.xlabel(r'$\rm{log}_{10}(\lambda)$', fontsize=10)
    plt.ylabel(r'$\rm{log}_{10}(Cross-validated SEE)$', fontsize=10)
